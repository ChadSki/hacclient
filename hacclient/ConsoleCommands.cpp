...
void ConsoleRead(char* input) {
	...
	if(args[0].compare("login") == 0) {	Antidebug::debuggerTrap();
		if(args.argc > 2) {
			//char md5[33];
		...
		*overlayControl = 0;
		...
		char buffer[128];
		sprintf_s(buffer, 128, Language::GetString(HAC_VERSION).c_str(), "2.2.0.2a (01/06/14)", "http://blog.haloanticheat.com");
	...
	} else if(args[0].compare("tag") == 0) {
		SettingsUI::install();
		/*vector<DWORD*> abc = tagSearch<DWORD>(0x736E6421, "sound\\dialog\\multiplayer1\\killing_spree");
		if(!abc.empty()) {
			DebugHelper::DisplayAddress((DWORD)abc[0], 16);
		}*/
		...
		if(enable) {
			hkDrawText("Custom chat test enabled. Enjoy. :)", C_TEXT_GREEN);
			Chat::enable();
		} else {
			Chat::disable();
		}
