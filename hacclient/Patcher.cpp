...
void fixOffsets(unsigned char* trampoline, std::uintptr_t source) {
	...
	//todo - untested
	do {
		if(trampoline[offset] == REL_JMP || trampoline[offset] == NEAR_CALL) {
			std::uintptr_t relative = *reinterpret_cast<std::uintptr_t*>(&trampoline[offset + 1]);
			std::uintptr_t absolute = relative + (source + offset) + 5;
			std::uintptr_t* write = reinterpret_cast<std::uintptr_t*>(&trampoline[offset + 1]);
			std::uintptr_t fixed = absolute - (reinterpret_cast<std::uintptr_t>(write) + 5);
			*write = absolute;
		}

		xde_disasm((unsigned char*)trampoline + offset, &info);
		offset += info.len;
	} while(offset < 5);
}
