...
const unsigned int MAX_TEXT_LINES = 6;
...
Optic::OpticAnimation generateDefaultAnimation();
Optic::OpticAnimation generateDefaultSlideAnimation();
Optic::OpticAnimation defaultAnimation = generateDefaultAnimation();
...
void queueText(LPDIRECT3DTEXTURE9 texture) {
	...
	while(text.size() > MAX_TEXT_LINES) {
		auto& medal = text[0];
...
Optic::OpticAnimation generateDefaultAnimation() {
	...
	animation.addKeyframe(Keyframe(7000, 1.0f), KEYFRAME_OPACITY);
	animation.addKeyframe(Keyframe(7250, 0.0f), KEYFRAME_OPACITY);
