...
template<typename T>
std::pair<int, std::unique_ptr<T>> OpticPack::fetchFile(HZIP handle, const std::string& file, bool text) {
	int index = -1;
	ZIPENTRY entry;
...
OpticPackMeta OpticPack::loadMeta(HZIP handle) {
	try {
		std::pair<int, std::unique_ptr<char>> buffer(fetchFile<char>(handle, "meta.txt", true));
		...
		throw OpticPackException(Language::GetString(OPTIC_PACK_META_MISSING));
	} catch(OpticPackException) {
		throw OpticPackException(Language::GetString(OPTIC_PACK_META_FAIL));
	}
}
...
std::string OpticPack::getLua() const {
	try {
		...
		throw OpticPackException(Language::GetString(OPTIC_PACK_LUA_FAIL));
	}
}
...
HZIP OpticPack::open(const std::string& fileName) {
	std::string fullPath = getWorkingDir() + "packs\\" + fileName;
	HZIP handle = OpenZip(reinterpret_cast<void*>(const_cast<char*>(fullPath.c_str())), 0, ZIP_FILENAME);

	if(handle == NULL) {
		throw OpticPackException(Language::GetString(OPTIC_PACK_OPEN_FAIL));
	}
