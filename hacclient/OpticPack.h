...
typedef std::pair<int, std::shared_ptr<const imageBin>> OpticImage;
typedef std::pair<int, std::shared_ptr<const audioBin>> OpticAudio;
...
class OpticPack {
	...
	template<typename T> static std::pair<int, std::unique_ptr<T>> fetchFile(HZIP handle, const std::string& file,
																			 bool text = false);
