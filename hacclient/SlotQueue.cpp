...
namespace {
	...
	bool hideSplash;
	bool joinQueue(const ServerInformation& info);
}

bool fullQueue(const char* response, const ServerInformation& info, bool _hideSplash) {
	setState(QUEUED);
...
bool fullQueue(const char* response, const ConnectionDetails* details) {
	ServerInformation info(details);
...
bool joinQueue(const ServerInformation& info) {
	ServerInformation* serverInfo = new ServerInformation(info);
...
void __cdecl queuePlayer(void* info) {
	...
	ServerInformation* serverInfo = static_cast<ServerInformation*>(info);
