...
OpticRender::OpticRender() {
	this->redirect = Preferences::find("optic_redirect", false);
	sprite = NULL;
...
LPDIRECT3DTEXTURE9 OpticRender::locateTexture(const OpticImage& image) {
	...
	return (textIt == textures.end())? createTexture(image) : textIt->second;
}
...
void OpticRender::setSpriteProperties(const std::string& name, bool loop, int rows, int columns, int fps, int blanks, int loopBegin) {
	try {
...
void OpticRender::addKeyframe(const std::string& name, const Keyframe& keyframe, TRANSFORM transform, TWEEN tween) {
	try {
...
void OpticRender::setMedalProperties(const std::string& name, int group, int priority, bool immune, int endTime) {
	try {
